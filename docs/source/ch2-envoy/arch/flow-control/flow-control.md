...
请注意，`stream`的 `readDisable(true)` 可能会被多个使用者重复调用。 当任何 `Filter`、`stream` 、`connection` 缓冲过多数据时，将调用 `stream`的 `readDisable(true)`  。 因此，`readDisable()` 会记住禁用`stream`的次数，并在每个调用者调用等次数的低水位线回调时恢复读取。 
